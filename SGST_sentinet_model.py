...
class SGST(torch.nn.Module):
    def __init__(self, args):
        ...
        self.price_concat_layernorm = torch.nn.LayerNorm(args.hidden_units * 6, eps=1e-8).cuda()
        self.price_lstm_layer = torch.nn.LSTM(input_size=args.hidden_units * 6, hidden_size=args.hidden_units,
    ...
    def log2feats(self, seqs, feature1, feature2, feature3, pos_seqs, neg_seqs):
        ...
        senti_emb_concat = torch.cat((pos_seqs, neg_seqs), -1)
        ...
        log_price = torch.cat((log_feats,log_feature1,log_feature2,log_feature3,pos_seqs,neg_seqs),-1)
    ...
    def forward(self, seqs, feature1, feature2, feature3,  pos_seqs, neg_seqs):
        ...
        pos_seqs, neg_seqs = torch.Tensor(pos_seqs).cuda(), torch.Tensor(neg_seqs).cuda()
        log_feats, seqs = self.log2feats(seqs, feature1, feature2, feature3,  pos_seqs, neg_seqs)

        return log_feats, seqs

    def predict(self, seqs, feature1, feature2, feature3,  pos_seqs, neg_seqs):
