...
author:Donglin Zhou
time:2021.12.15
function:基于市场情绪和技术面分析的股票价格预测
使用的情感数据集为stanfordNLP
'''
import os
import time
import argparse
from att_utils import *
from SGST_stanford_model import SGST
import torch
...
if __name__ == '__main__':
    # 读取数据的路径
    datafile_name = 'ST_MSFT_data_PCA.csv'
    file_name = './dataset/stockstats&stanford/' + datafile_name
    datatype = 'stanford'

    parser = argparse.ArgumentParser()
    parser.add_argument('--sparse_rate', default=3, type=int)       # 稀疏率，1,3,5
    parser.add_argument('--lr', default=0.001, type=float)
    parser.add_argument('--look_back', default=5, type=int)        # 5 10 15 20 25
    parser.add_argument('--hidden_units', default=50, type=int)      # 5 10 15 20 50
    parser.add_argument('--num_epochs', default=200, type=int)
    parser.add_argument('--batch_size', default=8, type=int)        # 5
    parser.add_argument('--num_heads', default=1, type=int)
    parser.add_argument('--num_blocks', default=2, type=int)        # 1 2 4
    ...
    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        for step in range(num_batch):
            ...
            v_Positive = X_train[step:step + args.batch_size, :, 3]
            Positive = X_train[step:step+args.batch_size, :, 4]
            confidence = X_train[step:step + args.batch_size, :, 5]
            Negative = X_train[step:step+args.batch_size, :, 6]
            v_Negative = X_train[step:step + args.batch_size, :, 7]
            ...
            pos_logits, seqs = model(y_seq, feature1, feature2, feature3, v_Positive, Positive,confidence, Negative, v_Negative)
        ...
        if epoch % 20 == 0:
            ...
            mse, mape, smape = evaluate_test(model, X_test, y_test, datatype)
